  ...
  function getCheckedFiles() {
    ...
    if (!isShare()) {
      if (PAGE_INFO.vmode == 'list') {
        $('span.EOGexf').parent().each(function () {
          if (getDefaultStyle($(this).find('>span>span').get(0), 'display') != 'none') {
        ...
        $('div.cEefyz').each(function () {
          if (getDefaultStyle($(this).find('>span').get(0), 'display') != 'none') {
            var fileName = $(this).find('div.file-name>a').text();
            checkedFiles.push(fileName);
          }
        });
      }
      ...
      if (PAGE_INFO.path == '/'
        ...
        $(listType + '.JS-item-active').each(function () {
          checkedFiles.push($(this).find('a.filename').text());
        });
      }
    }
    return checkedFiles;
